class UserActivity
  ...
  def self.all_users_by_date(year, month=nil, day=nil)
    Activity.by_year(year).by_month(month)
  end

  def self.all_by_date(username, year, month=nil, day=nil)
    user_id = User.select(:id).find_by(username: username)

    UserActivity.all_users_by_date(year, month, day).where(user_id: user_id)
  end
